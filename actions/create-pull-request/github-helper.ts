...
export class GitHubHelper {
  ...
  private async createOrUpdate(
    ...
    try {
      ...
      if (!e.message || !e.message.includes(`A pull request already exists for ${headBranch}`)) {
        throw e
      }
    }
  ...
  async createOrUpdatePullRequest(
    ...
    const updateIssueParams: { milestone?: number; labels?: string[]; assignees?: string[] } = {}
    if (inputs.milestone) {
    ...
    const requestReviewersParams: { reviewers?: string[]; team_reviewers?: string[] } = {}
