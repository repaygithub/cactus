...
export async function getWorkingBaseAndType(
  ...
  const symbolicRefResult = await git.exec(['symbolic-ref', 'HEAD', '--short'], true)
...
async function isAhead(git: GitCommandManager, branch1: string, branch2: string): Promise<boolean> {
  const result = await git.revList([`${branch1}...${branch2}`], ['--right-only', '--count'])
...
async function isBehind(
  ...
  const result = await git.revList([`${branch1}...${branch2}`], ['--left-only', '--count'])
  return Number(result) > 0
}

// Return true if branch2 is even with branch1
async function isEven(git: GitCommandManager, branch1: string, branch2: string): Promise<boolean> {
  return !(await isAhead(git, branch1, branch2)) && !(await isBehind(git, branch1, branch2))
}
...
export async function createOrUpdateBranch(
  ...
  if (workingBase != base) {
    ...
    const commits = await git.revList([`${workingBase}..${tempBranch}`, '.'], ['--reverse'])
