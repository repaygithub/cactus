...
export async function createPullRequest(inputs: Inputs): Promise<void> {
  ...
  try {
    ...
    const branchRepository = inputs.pushToFork ? inputs.pushToFork : baseRemote.repository
    if (inputs.pushToFork) {
      // Check if the supplied fork is really a fork of the base
      const parentRepository = await githubHelper.getRepositoryParent(branchRepository)
    ...
    if (inputs.branchSuffix) {
      switch (inputs.branchSuffix) {
        case 'short-commit-hash':
          ...
          inputs.branch = `${inputs.branch}-${await git.revParse('HEAD', ['--short'])}`
          break
        case 'timestamp':
    ...
    if (['created', 'updated'].includes(result.action)) {
      ...
      await git.push(['--force-with-lease', branchRemoteName, `HEAD:refs/heads/${inputs.branch}`])
      core.endGroup()
    }
    ...
    if (result.hasDiffWithBase) {
      ...
      await githubHelper.createOrUpdatePullRequest(inputs, baseRemote.repository, branchRepository)
    } else {
      ...
      if (['updated', 'not-updated'].includes(result.action)) {
        ...
        if (inputs.deleteBranch) {
          ...
          await git.push(['--delete', '--force', branchRemoteName, `refs/heads/${inputs.branch}`])
        }
      }
    }
  } catch (error) {
