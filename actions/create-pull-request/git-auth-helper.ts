...
export class GitAuthHelper {
  ...
  private async setExtraheaderConfig(extraheaderConfigValue: string): Promise<void> {
    ...
    await this.git.config(this.extraheaderConfigKey, this.extraheaderConfigPlaceholderValue)
  ...
  private async getAndUnset(): Promise<string> {
    let configValue = ''
    // Save and unset persisted extraheader credential in git config if it exists
    if (await this.git.configExists(this.extraheaderConfigKey, this.extraheaderConfigValueRegex)) {
      configValue = await this.git.getConfigValue(
        this.extraheaderConfigKey,
        this.extraheaderConfigValueRegex
      )
      if (
        await this.git.tryConfigUnset(this.extraheaderConfigKey, this.extraheaderConfigValueRegex)
      ) {
  ...
  private async gitConfigStringReplace(find: string, replace: string): Promise<void> {
