...
const fillTextField =
  ...
  async (label: string, text: string): Promise<void> => {
    const field = queryByLabelText(label)
    await t.click(field).typeText(field, text)
  }

const selectDropdownOption =
  (t: TestController): ((label: string, optionOrOptions: string | string[]) => Promise<void>) =>
  async (label: string, optionOrOptions: string | string[]): Promise<void> => {
