...
test('navigate to account page using SplitButton', async (t: TestController): Promise<void> => {
  await t.click(queryAllByText('View Account').nth(0))
  await t.expect(queryByText('Account 85963').exists).ok()
})

test('delete account with using SplitButton', async (t: TestController): Promise<void> => {
