...
Collecting data using forms is one of the most basic functions of a website. [@repay/cactus-web](/story/cactus-web-intro--page) provides several styled form inputs, as well as a few custom inputs that emulate built-in HTML inputs. 
This guide will show you how to use this library, but will also have links out to [Final Form](https://final-form.org/) when appropriate for aspects that are unaltered.
...
The basic Final Form library is very flexible, and we highly recommend going through the docs to understand what it's capable of. The same is true for React Final Form. However, the majority of use cases can be covered by just the components in Cactus Form.
