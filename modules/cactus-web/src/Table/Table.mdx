...
### Sticky Right Column 
For the default styling, there is the possibility to set the right most column right to be sticky. This column stays in place as the user scrolls through the table. For enabling this feature, the `Table` component allows a `stickyRightColumn` prop.
