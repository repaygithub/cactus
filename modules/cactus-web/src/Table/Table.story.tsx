...
tableStories.add(
  ...
  (): React.ReactElement => {
    ...
    const alignment = createAlignKnob()
    const hasBody = boolean('Has tbody', true)
    const variant = select('Variant Override', varOptions, 'card') as Variant
    ...
    for (let i = 0; i < rowCount; i++) {
      i === 2
        ? rows.push(makeRow(`${cellText} ${veryLongText}`, i + 1))
        : rows.push(makeRow(cellText, i + 1))
    }
