...
const BreakpointBox = styled(Box)`
  ...
  // Each value in `mediaQueries` is a MediaQueryList object;
  // use the `media` property to get the text of the query.
  @media ${p => p.theme.mediaQueries.small.media} {
    border-radius: 25%;
  }
  // @repay/cactus-theme has helpers to shorten common theme property access.
  ${mediaGTE('medium')} {
    border-radius: 35%;
  }
  ${mediaGTE('large')} {
