...
- You can control the values using one of two methods, which should not be mixed:
  - Pass `value` or `defaultValue` to `CheckBoxGroup`. These should be objects, mapping the checkbox names to their boolean values.
...
<PropsTable of={CheckBoxGroup} includeProps={['name', 'label', 'disabled', 'tooltip', 'error', 'warning', 'success']} />
