...
The `ScreenSizeProvider` gives a programatic way of determining what the current "size" of the screen is, according to the theme's `mediaQueries` object. The possible sizes are directly from the media queries object ("tiny", "small", "medium", "large", "extraLarge"). Because it uses `theme.mediaQueries`, it should be nested inside a `StyleProvider`; otherwise it will always give the default size of "large".
