...
- Just like the `Table`, the `DataGrid` should be rendered as a child of `ScreenSizeProvider` to take advantage of the card carousel feature.
...
The `DataGrid` component has two sub-components to help define your grid structure:
  ...
  - `DataGrid.Column` - Receives a child render function which will be to render anything in this column's cells. The column will not
    have a title, and can be used to render components that allow you to take some action for a given row.
